...
def selected_game() -> JSON:
    ...
    recommendations_per_game = defaultdict(int)

    if request.method == 'POST':
        data = request.json
        game_set = set(data["games"])
        tags = set()

        for game in game_set:
            related = find_closest(GAME_WEIGHTS[GAME_INDEX[game]])
            tags |= set(GAME_TAGS[game])
            closest = _remove_self(game, related)
            for game, score in closest:
                if game in closest:
                    continue
                recommendations_per_game[game] += 1
                recommendations[game] += float(score)

        return jsonify({
            'related': sorted([(recommendations, score / recommendations_per_game[recommendations]) for recommendations, score in recommendations.items()], key=lambda x: x[1], reverse=True),
            'tags': list(tags),
        })
